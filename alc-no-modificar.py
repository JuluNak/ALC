...
def afin(theta, s, b):
    res: list = []
    res.append(np.array([rotayescala(theta, s)[0][0],
               rotayescala(theta, s)[0][1], b[0]]))
    res.append(np.array([rotayescala(theta, s)[1][0],
               rotayescala(theta, s)[1][1], b[1]]))
...
def pinvEcuacionesNormales(X, L, Y):
    ...
    if n > p:
        ...
        for j in range(n):
            b = XT[:, j]
            # aca es donde por cada columna uso la func
            x_sol = res_tri(L, b, "inferior")
            for k in range(p):
                B[k][j] = x_sol[k]  # aca lo agrego a la matriz
        # ACA ES LA CREACION DE U A PARTIR DE LT.U=B
        filasU = dimxt[0]
        columnasU = dimxt[1]
        U = np.zeros((filasU, columnasU), dtype=float)
...
def pinvSVD(U, S, V, Y):
    ...
    # Calculo X+
    X_plus = multiplicacionMatricial(
        multiplicacionMatricial(V, S_plus), traspuesta(U))
...
L2 = cholesky(multiplicacionMatricial(X2, traspuesta(X2)))
# print(L)
print(pinvEcuacionesNormales(X2, L2, Y))
